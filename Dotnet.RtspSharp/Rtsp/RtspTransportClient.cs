...
using Dotnet.RtspSharp.Rtsp.Authentication;
...
namespace Dotnet.RtspSharp.Rtsp
{
    abstract class RtspTransportClient : IRtspTransportClient
    {
        ...
        {
            LogMangerNative.Instance.Info($"ExecuteRequest Try, Info : {requestMessage.Method.ToString()}, {requestMessage.ConnectionUri.AbsoluteUri}, {requestMessage.UserAgent}");
            RtspResponseMessage responseMessage = await ExecuteRequest(requestMessage, token, responseReadPortionSize);

            LogMangerNative.Instance.Info($"ExecuteRequest Completed : {responseMessage.StatusCode.ToString()}");
            ...
            CancellationToken token, int responseReadPortionSize = 0)
        {
            ...
            LogMangerNative.Instance.Info("ExecuteRequest GetResponseAsync");
            RtspResponseMessage responseMessage = await GetResponseAsync(responseReadPortionSize);


            LogMangerNative.Instance.Info($"GetResponseAsync Completed : {responseMessage.StatusCode.ToString()}");
            if (responseMessage.StatusCode != RtspStatusCode.Unauthorized)
                return responseMessage;

            LogMangerNative.Instance.Info($"GetResponseAsync User Info : {ConnectionParameters?.Credentials?.UserName}, {ConnectionParameters?.Credentials?.Password}");
            ...
            LogMangerNative.Instance.Info($"GetResponseAsync authenticateHeader : {authenticateHeader}");
            ...
            LogMangerNative.Instance.Info($"UpdateSequenceNumber requestMessage : {requestMessage.ToString()}");

            /////////////////////////////////////////////////////////////////////
            await SendRequestAsync(requestMessage, token);
            responseMessage = await GetResponseAsync();
            LogMangerNative.Instance.Info($"GetResponseAsync responseMessage : {responseMessage.StatusCode.ToString()}");
        ...
        private async Task<RtspResponseMessage> GetResponseAsync(int responseReadPortionSize = 0)
        {
            ...
            LogMangerNative.Instance.Info($"GetResponseAsync startOfResponse : {startOfResponse}");
            ...
            LogMangerNative.Instance.Info($"GetResponseAsync endOfResponseHeaders : {endOfResponseHeaders}");
            ...
            LogMangerNative.Instance.Info($"GetResponseAsync rtspResponseMessage : ({rtspResponseMessage.StatusCode.ToString()})");
            ...
            LogMangerNative.Instance.Info($"GetResponseAsync contentLength : ({contentLength})");
        ...
        private void AddAuthorizationHeader(RtspRequestMessage request)
        {
            ...
            LogMangerNative.Instance.Info($"AddAuthorizationHeader headerValue : ({headerValue})");
