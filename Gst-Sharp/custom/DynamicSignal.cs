...
namespace Gst {
	...
	public static class DynamicSignal {
                       ...
                       IntPtr ihint, IntPtr data)
        {
            ...
            if (k == null)
            {
                Console.Error.WriteLine("Key is null, cannot lookup in SignalHandlers dictionary");
                return;
            }

            if (SignalHandlers.ContainsKey(k))
            {
                SignalInfo si = (SignalInfo)SignalHandlers[k];
                if (si != null && si.RegisteredHandler != null)
                {
                    GLib.SignalArgs arg = (GLib.SignalArgs)Activator.CreateInstance(si.ArgsType);
                    arg.Args = args;
                    si.RegisteredHandler?.DynamicInvoke(new object[] { o, arg });
                ...
                {
                    Console.Error.WriteLine("SignalInfo object or RegisteredHandler is null");
                }
            }
            else
            {
                Console.Error.WriteLine("Key not found in SignalHandlers dictionary");
            }
        }
